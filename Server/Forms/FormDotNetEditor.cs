    ...
    public partial class FormDotNetEditor : Form
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (Program.form1.listView1.SelectedItems.Count > 0)
            {
                ...
                MessageBox.Show("Executed!", "AuroraRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("Selected client = 0", "AuroraRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
...
";
        ...
        {
            if (listBoxReferences.Items.Count == 0 || string.IsNullOrWhiteSpace(txtBox.Text)) return;
            if (!txtBox.Text.ToLower().Contains("try") && !txtBox.Text.ToLower().Contains("catch")) MessageBox.Show("Please add try catch", "AuroraRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            List<string> reference = new List<string>();
        ...
        private void Compiler(CodeDomProvider codeDomProvider, string source, string[] referencedAssemblies)
        {
            try
            {
                ...
                if (compilerResults.Errors.Count > 0)
                {
                    ...
                    {
                        MessageBox.Show(string.Format("{0}\nLine: {1}", compilerError.ErrorText, compilerError.Line), "AuroraRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                    }
                }
                else
                {
                    ...
                    MessageBox.Show("No Error!", "AuroraRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "AuroraRAT | Dot Net Editor", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            finally
            {
                //GC.Collect();
